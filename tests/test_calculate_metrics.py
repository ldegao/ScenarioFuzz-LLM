...
测试覆盖：
1. calculate_metrics_from_scenarios函数的基本功能
2. 从场景数据计算累积指标
3. 增量计算功能
4. 重新计算功能
5. 错误处理
...
def check_carla_environment():
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"[INFO] CARLA container {docker_name} does not exist")
            # 尝试启动CARLA容器
            script_dir = PROJECT_ROOT / "script"
            from experiments.core.environment_manager import ensure_carla_running, wait_for_port
            try:
        ...
        # 检查端口是否可用（CARLA可能使用2000或4000端口）
        ports_to_check = [2000, 4000]
        ...
        # 即使端口不可访问，如果容器在运行，我们也允许测试继续
        # 因为CARLA可能需要一些时间启动，或者端口检查可能不准确
        if result.returncode == 0 and result.stdout.strip() == "running":
            print("[INFO] CARLA container is running, allowing test to proceed")
            return True
        return False
    except Exception as e:
...
class TestCalculateMetrics(unittest.TestCase):
    ...
    def test_calculate_metrics_basic(self):
        ...
        self.assertIn('num_skipped', result)
    ...
    def test_output_files_created(self):
        ...
        self.assertTrue(metrics_dir.exists(), "metrics directory should be created")
        ...
        with open(records_path, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                self.assertIn('generation_id', record)
                self.assertIn('scenario_id', record)
                self.assertIn('pc', record)
                self.assertIn('pec', record)
                self.assertIn('tcd', record)
                self.assertIn('bcm', record)
                self.assertIn('num_accumulated_scenarios', record)
        
        # 验证summary文件内容
        with open(summary_path, 'r', encoding='utf-8') as f:
            summary = json.load(f)
            self.assertIn('num_records', summary)
            self.assertIn('pc', summary)
            self.assertIn('pec', summary)
            self.assertIn('tcd', summary)
            self.assertIn('bcm', summary)
    ...
    def test_incremental_calculation(self):
        ...
        # 验证文件没有被重复写入（记录数应该还是3）
        metrics_dir = self.experiment_dir / "metrics"
        records_path = metrics_dir / "metrics_records.jsonl"
        
        with open(records_path, 'r', encoding='utf-8') as f:
            lines = [line.strip() for line in f if line.strip()]
            self.assertEqual(len(lines), 3, "Should still have 3 records after incremental calculation")
    ...
    def test_recalculate(self):
        ...
        # 应该重新计算所有场景
        self.assertEqual(result2['num_calculated'], 3)
        ...
        with open(records_path, 'r', encoding='utf-8') as f:
            lines = [line.strip() for line in f if line.strip()]
            self.assertEqual(len(lines), 3, "Should have 3 records after recalculate (overwritten)")
    
    def test_custom_output_dir(self):
        """测试自定义输出目录"""
        custom_output_dir = self.temp_path / "custom_metrics"
        ...
        # 验证文件在自定义目录中创建
        records_path = custom_output_dir / "metrics_records.jsonl"
        summary_path = custom_output_dir / "metrics_summary.json"
        
        self.assertTrue(records_path.exists(), "Records should be in custom output dir")
        self.assertTrue(summary_path.exists(), "Summary should be in custom output dir")
    
    def test_empty_experiment_dir(self):
        """测试空实验目录的处理"""
        empty_dir = self.temp_path / "empty_experiment"
        empty_dir.mkdir()
        
        result = calculate_metrics_from_scenarios(
            experiment_dir=empty_dir,
            incremental=False,
            recalculate=False
        )
        
        # 应该返回空结果
        self.assertEqual(result['num_scenarios'], 0)
        self.assertEqual(result['num_calculated'], 0)
        self.assertEqual(result['num_skipped'], 0)
    
    def test_nonexistent_experiment_dir(self):
        """测试不存在的实验目录"""
        nonexistent_dir = self.temp_path / "nonexistent"
        
        with self.assertRaises(ValueError):
            calculate_metrics_from_scenarios(
                experiment_dir=nonexistent_dir,
                incremental=False,
                recalculate=False
            )
